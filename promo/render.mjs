...
const frames = fs.readdirSync('appframes').length;
await p.goto(`http://localhost:8001/compose.html?frames=${frames}`);
...
const list = only ? only.map(s => Math.round(s * FPS)) : [...Array(FPS * DUR).keys()];
for (const f of list) {
